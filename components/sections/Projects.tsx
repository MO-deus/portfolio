        ...
        <div className={styles.filters} role="group" aria-label="Filter projects by tag">
          {allTags.map((tag) => (
            <button
              ...
              id={`projects-filter-${tag.toLowerCase()}`}
            >
              {tag}
            </button>
          ))}
          ...
          {filtered.map((project) => (
            <div key={project.id} className={`glass-card ${styles.card}`}>
              ...
              <div className={styles.cardBody}>
                ...
                {/* Tech */}
                <div className="tech-pills" style={{ marginTop: "auto", paddingTop: "var(--space-md)" }}>
                  {project.tech.slice(0, 5).map((t) => (
                    <span key={t} className="chip chip-neutral">{t}</span>
                  ))}
                  {project.tech.length > 5 && (
                    <span className="chip chip-neutral">+{project.tech.length - 5}</span>
                  )}
                </div>
                ...
                {project.isInternal && (
                  <div className={styles.internalNote}>
                    ...
                    Internal project — shipped at Fanon
                  </div>
                )}
              </div>
            </div>
